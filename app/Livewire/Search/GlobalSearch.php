...
class GlobalSearch extends Component
{
    ...
    protected function performSearch(): void
    {
        ...
        // Search RFQs
        if ($user->hasPermission('view_rfqs')) {
            $rfqs = Request::query()
        ...
        // Search Markets
        if ($user->hasPermission('view_markets')) {
    ...
    protected function addNavigationShortcuts(string $query): void
    {
        ...
        $matchedShortcuts = collect($shortcuts)
            ->filter(function ($shortcut, $key) use ($query) {
                $hasPermission = !isset($shortcut['permission']) || auth()->user()->hasPermission($shortcut['permission']);
