<div>
    <x-card>
        ...
        <div class="mt-6 flex gap-2">
            <x-button icon="arrow-left" href="{{ route('seller.markets.index') }}">@lang('Markets')</x-button>
            <x-button icon="pencil" wire:click="$dispatch('seller::load::market', { market: '{{ $market->id }}'})">{{ __('Update Market: #:id', ['id' => $market->id]) }}</x-button>
        </div>
                ...
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach($this->products as $product)
                        <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 flex flex-col gap-3 bg-white dark:bg-dark-700">
                            ...
                            <x-button size="sm" icon="arrow-top-right-on-square" href="{{ route('seller.products.show', $product) }}" class="self-start">
                                @lang('View Product')
                            </x-button>
                        </div>
                    @endforeach
